...
def _png1_spark_ast():
    ...
    fig.suptitle("Spark AST Lineage Parser — Source → Transform → Sink",
    ...
    ax_code.text(0.04, 0.96, code, transform=ax_code.transAxes,
                 fontfamily="monospace", fontsize=8.5, color="#c9d1d9",
    ...
    nx.draw_networkx_nodes(G, pos, ax=ax_g, node_color=node_colors, node_size=node_sizes)
    nx.draw_networkx_edges(G, pos, ax=ax_g, edge_color="#555", arrows=True,
    ...
    ax_g.legend(handles=legend, loc="lower right", fontsize=8,
                facecolor=CARD_BG, labelcolor="white", framealpha=0.8)
...
def _png2_dbt_lineage():
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    fig.patch.set_facecolor(DARK_BG)
    fig.suptitle("dbt Lineage Parser — ref() / source() Dependency Graph",
                 color="white", fontsize=14, fontweight="bold", y=0.97)

    ax_sql = axes[0]
    ax_sql.set_facecolor(CARD_BG)
    ax_sql.axis("off")
    ax_sql.set_title("dbt Model SQL (orders_enriched)", color="#90caf9", fontsize=11, pad=8)
    dbt_sql = (
    ...
    ax_sql.text(0.04, 0.96, dbt_sql, transform=ax_sql.transAxes,
                fontfamily="monospace", fontsize=8.5, color="#c9d1d9",
                verticalalignment="top", linespacing=1.6)
    ...
    ax_g.set_facecolor(DARK_BG)
    ax_g.axis("off")
    ax_g.set_title("Extracted Model Dependency Graph", color="#90caf9", fontsize=11, pad=8)
    ...
    nx.draw_networkx_edges(G, pos, ax=ax_g, edge_color="#555", arrows=True, arrowsize=15, width=1.5)
    nx.draw_networkx_labels(G, pos, {n: n for n in G.nodes}, ax=ax_g, font_size=7.5, font_color="white")
...
def _png3_cross_system():
    ...
    ax.set_title("Cross-System Unified Lineage Graph — SQL + Spark + dbt + Airflow",
    ...
    edge_colors = [
        "#E74C3C" if G.edges[e].get("system") == "cross" else
        SYSTEM_COLORS.get(G.edges[e].get("system", "sql"), "#888")
        for e in G.edges
    ]
    edge_styles = [
        "dashed" if G.edges[e].get("system") == "cross" else "solid"
        for e in G.edges
    ]
    ...
    for (src, tgt), ec, style in zip(G.edges, edge_colors, edge_styles):
        nx.draw_networkx_edges(
            ...
            edge_color=[ec], arrows=True, arrowsize=14,
            width=2.0 if style == "dashed" else 1.2,
            style=style, connectionstyle="arc3,rad=0.08",
        )
...
def _png4_impact():
    ...
    fig.patch.set_facecolor(DARK_BG)
    fig.suptitle('Impact Propagation — Blast Radius: "orders.amount" renamed',
                 color="white", fontsize=14, fontweight="bold", y=0.97)
    ...
    sev_colors = {"BREAKING": "#E74C3C", "WARNING": "#F5A623", "OK": "#7ED321", "SOURCE": "#4A90D9"}
    ...
    nx.draw_networkx_edges(G, pos, ax=ax_g, edge_color="#555", arrows=True, arrowsize=14, width=1.5)
    ...
    ax_t.text(0.06, 0.96, "\n".join(report_lines),
              ...
              fontsize=9.5, color="#c9d1d9", verticalalignment="top", linespacing=1.6)
